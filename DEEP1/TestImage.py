...
def getSession():
    ...
    MODEL_DIR = './DEEP1/savegraphface/'
    file_name = "my_test_model.ckpt-200"
    saver.restore(sess, MODEL_DIR+file_name)

    return sess,label_pred,predictions,image_input


def getLabelImage(filename,sess,label_pred,predictions,image_input):
...
def getLabel():
    with open("./DEEP1/label.txt", "r") as ins:
        array = []
        for line in ins:
            array.append(line)
    return array
